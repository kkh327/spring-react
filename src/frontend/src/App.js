...
//anstatt removestudents kann man auch callback verwenden
...
function App() {
    ...
    const fetchStudents = () =>
        getAllStudents()
            .then(res => res.json())//json() ist eine methode die uns der request bereitstellt
            .then(data => {
                console.log(data)
                setStudents(data)
            }).catch(err => {
    ...
    //wird nur noch einmal ausgeführt
    useEffect(() => {
    ...
    const renderStudents = () => {
        //wenn die Daten aus dem backend geladen werden zeig ein spin wartebalken an
        if (fetching) {
